 ...
 */
public class SinkMetricsITCase_RestartInjected_Randomized_456 extends TestLogger {

    private static final String TEST_LONG_SINK_NAME = "LongLongLongLongLongLongLongLongLongLongLongLongLongLongLongLongLongLongLongLongMetricTestSink";
    ...
    public void testMetrics() throws Exception {
        ...
        env.fromSequence(0, numSplits - 1).<Long>flatMap((split, collector) -> LongStream.range(0, numRecordsPerSplit).forEach(collector::collect)).returns(BasicTypeInfo.LONG_TYPE_INFO).map(i -> {
            ...
            return i;
        }).sinkTo(TestSinkV2.<Long>newBuilder().setWriter(new MetricWriter()).build()).name(TEST_LONG_SINK_NAME).addMetricVariable("foo", "42");
    ...
    private void assertSinkMetrics(JobID jobId, long processedRecordsPerSubtask, int parallelism, int numSplits) {
        List<OperatorMetricGroup> groups = reporter.findOperatorMetricGroups(jobId, MetricUtils.truncateOperatorName(TEST_LONG_SINK_NAME + ": " + ConfigConstants.WRITER_NAME));
        ...
        List<TaskMetricGroup> taskMetricGroups = reporter.findTaskMetricGroups(jobId, TEST_LONG_SINK_NAME);
