...
public class EventTimeAllWindowCheckpointingITCase_RestartInjected_Randomized_456 extends TestLogger {
    ...
    public void testPreAggregatedTumblingTimeWindow() throws Exception {
        ...
        RestartStrategyUtils.configureFixedDelayRestartStrategy(env, 1, 0L);
        RestartFramework.at("during_window_processing").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        env.addSource(new FailingSource(new EventTimeWindowCheckpointingITCase.KeyedEventTimeGenerator(numKeys, windowSize), numElementsPerKey)).rebalance().windowAll(TumblingEventTimeWindows.of(Duration.ofMillis(windowSize))).reduce(new ReduceFunction<Tuple2<Long, IntType>>() {
        ...
        RestartFramework.at("after_checkpoint").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        JobClient jobClient = env.executeAsync("PreAggregated Tumbling Window Test");
        Thread.sleep(200);
        Thread.sleep(300);
        jobClient.getJobExecutionResult().get();
    }
    ...
    public void testPreAggregatedSlidingTimeWindow() throws Exception {
        ...
        env.enableCheckpointing(100);
        RestartStrategyUtils.configureFixedDelayRestartStrategy(env, 1, 0L);
        env.addSource(new FailingSource(new EventTimeWindowCheckpointingITCase.KeyedEventTimeGenerator(numKeys, windowSlide), numElementsPerKey)).rebalance().windowAll(SlidingEventTimeWindows.of(Duration.ofMillis(windowSize), Duration.ofMillis(windowSlide))).reduce(new ReduceFunction<Tuple2<Long, IntType>>() {
        ...
        JobClient jobClient = env.executeAsync("PreAggregated Sliding Window Test");
        Thread.sleep(200);
        RestartFramework.at("during_window_processing").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(300);
        jobClient.getJobExecutionResult().get();
