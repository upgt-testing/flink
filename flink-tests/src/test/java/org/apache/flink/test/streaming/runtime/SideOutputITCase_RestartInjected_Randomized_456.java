 ...
 */
public class SideOutputITCase_RestartInjected_Randomized_456 extends AbstractTestBaseJUnit4 implements Serializable {
    ...
    public void testWatermarkForwarding() throws Exception {
        ...
        DataStream<Integer> dataStream = env.fromSource(createTimestampedWatermarkSource(), WatermarkStrategy.<Integer>noWatermarks(), "timestamped-source").setParallelism(1);
        ...
        JobClient jobClient = env.executeAsync("Watermark Forwarding Test");
        Thread.sleep(1000);
    ...
    public void testSideOutputWithMultipleConsumers() throws Exception {
        ...
        RestartFramework.at("after_job_submit").on(MINI_CLUSTER_RESOURCE).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        JobClient jobClient = env.executeAsync("Side Output Multiple Consumers Test");
        Thread.sleep(1000);
        jobClient.getJobExecutionResult().get();
    ...
    public void testSideOutputWithMultipleConsumersWithObjectReuse() throws Exception {
        ...
        passThroughtStream.getSideOutput(sideOutputTag).addSink(sideOutputResultSink2);
        passThroughtStream.addSink(resultSink);
        JobClient jobClient = env.executeAsync("Side Output Object Reuse Test");
        Thread.sleep(1000);
        RestartFramework.at("during_job_running").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        jobClient.getJobExecutionResult().get();
        assertEquals(Arrays.asList("sideout-1", "sideout-2", "sideout-3", "sideout-4", "sideout-5"), sideOutputResultSink1.getSortedResult());
    ...
    public void testDifferentSideOutputTypes() throws Exception {
        ...
        passThroughtStream.addSink(resultSink);
        JobClient jobClient = env.executeAsync("Different Side Output Types Test");
        Thread.sleep(1000);
        jobClient.getJobExecutionResult().get();
        assertEquals(Arrays.asList("sideout-1", "sideout-2", "sideout-3", "sideout-4", "sideout-5"), sideOutputResultSink1.getSortedResult());
    ...
    public void testKeyedProcessFunctionSideOutput() throws Exception {
        ...
        SingleOutputStreamOperator<Integer> passThroughtStream = dataStream.keyBy(new KeySelector<Integer, Integer>() {
            ...
            @Override
            public void processElement(Integer value, KeyedProcessFunction<Integer, Integer, Integer>.Context ctx, Collector<Integer> out) {
                out.collect(value);
                ctx.output(sideOutputTag, "sideout-" + String.valueOf(value));
            }
        });
    ...
    private static class TestWatermarkAssigner implements WatermarkStrategyWithPunctuatedWatermarks<Integer> {
    ...
    public void testAllWindowLateArrivingEvents() throws Exception {
        ...
        SingleOutputStreamOperator<Integer> windowOperator = dataStream.assignTimestampsAndWatermarks(new TestWatermarkAssigner()).windowAll(SlidingEventTimeWindows.of(Duration.ofMillis(1), Duration.ofMillis(1))).sideOutputLateData(lateDataTag).apply(new AllWindowFunction<Integer, Integer, TimeWindow>() {
    ...
    public void testKeyedWindowLateArrivingEvents() throws Exception {
        ...
        SingleOutputStreamOperator<String> windowOperator = dataStream.assignTimestampsAndWatermarks(new TestWatermarkAssigner()).keyBy(new TestKeySelector()).window(SlidingEventTimeWindows.of(Duration.ofMillis(1), Duration.ofMillis(1))).allowedLateness(Duration.ofMillis(2)).sideOutputLateData(lateDataTag).apply(new WindowFunction<Integer, String, Integer, TimeWindow>() {
    ...
    public void testProcessdWindowFunctionSideOutput() throws Exception {
        ...
        SingleOutputStreamOperator<Integer> windowOperator = dataStream.assignTimestampsAndWatermarks(new TestWatermarkAssigner()).keyBy(new TestKeySelector()).window(SlidingEventTimeWindows.of(Duration.ofMillis(1), Duration.ofMillis(1))).process(new ProcessWindowFunction<Integer, Integer, Integer, TimeWindow>() {
    ...
    public void testProcessAllWindowFunctionSideOutput() throws Exception {
        ...
        SingleOutputStreamOperator<Integer> windowOperator = dataStream.assignTimestampsAndWatermarks(new TestWatermarkAssigner()).windowAll(SlidingEventTimeWindows.of(Duration.ofMillis(1), Duration.ofMillis(1))).process(new ProcessAllWindowFunction<Integer, Integer, TimeWindow>() {
    ...
    private static AbstractTestSource<Integer> createTimestampedWatermarkSource() {
        return new AbstractTestSource<>() {
            ...
            public SourceReader<Integer, TestSplit> createReader(SourceReaderContext ctx) {
                return new TestSourceReader<Integer>(ctx) {
                    ...
                    public InputStatus pollNext(ReaderOutput<Integer> out) {
                        if (!emitted) {
                            out.collect(1, 0);
                            out.emitWatermark(new org.apache.flink.api.common.eventtime.Watermark(0));
                            out.collect(2, 1);
                            out.collect(5, 2);
                            out.emitWatermark(new org.apache.flink.api.common.eventtime.Watermark(2));
                            out.collect(3, 3);
                            out.collect(4, 4);
                            emitted = true;
            ...
            @Override
            public SplitEnumerator<TestSplit, Void> createEnumerator(SplitEnumeratorContext<TestSplit> context) {
