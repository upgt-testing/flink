...
public class UdfStreamOperatorCheckpointingITCase_RestartInjected_Randomized_456 extends StreamFaultToleranceTestBase {
    ...
    public void testProgram(StreamExecutionEnvironment env) {
        // base stream
        KeyedStream<Tuple2<Integer, Long>, Integer> stream = env.addSource(new StatefulMultipleSequence()).keyBy(x -> x.f0);
        stream.// testing built-in aggregate
        min(1).// failure generation
        map(new OnceFailingIdentityMapFunction(NUM_INPUT)).keyBy(x -> x.f0).addSink(new MinEvictingQueueSink());
        stream.// testing UDF reducer
        ...
        }).keyBy(x -> x.f0).addSink(new SumEvictingQueueSink());
    }
    ...
    public void runCheckpointedProgram() throws Exception {
        ...
        JobID jobId = cluster.getClusterClient().submitJob(jobGraph).get();
        Thread.sleep(2000);
        ...
        Thread.sleep(2000);
        try {
            cluster.getClusterClient().requestJobResult(jobId).get().toJobExecutionResult(getClass().getClassLoader());
        } catch (Exception e) {
    ...
    private static class OnceFailingIdentityMapFunction extends RichMapFunction<Tuple2<Integer, Long>, Tuple2<Integer, Long>> implements ListCheckpointed<Long> {
        ...
        public void open(OpenContext openContext) throws Exception {
            long failurePosMin = (long) (0.4 * numElements / getRuntimeContext().getTaskInfo().getNumberOfParallelSubtasks());
            long failurePosMax = (long) (0.7 * numElements / getRuntimeContext().getTaskInfo().getNumberOfParallelSubtasks());
