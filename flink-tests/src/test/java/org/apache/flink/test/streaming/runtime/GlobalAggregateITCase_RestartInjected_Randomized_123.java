 ...
 */
public class GlobalAggregateITCase_RestartInjected_Randomized_123 extends AbstractTestBaseJUnit4 {

    @Test
    public void testSuccessfulUpdateToGlobalAggregate() throws Exception {
        RestartFramework.at("during_global_aggregate").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        StreamExecutionEnvironment streamExecutionEnvironment = StreamExecutionEnvironment.getExecutionEnvironment();
        streamExecutionEnvironment.addSource(new TestSourceFunction(new IntegerAggregateFunction(), false)).sinkTo(new DiscardingSink<>());
        JobClient jobClient = streamExecutionEnvironment.executeAsync();
        Thread.sleep(1000);
    ...
    public void testExceptionThrowingAggregateFunction() throws Exception {
        RestartFramework.at("after_job_submit").on(MINI_CLUSTER_RESOURCE).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        StreamExecutionEnvironment streamExecutionEnvironment = StreamExecutionEnvironment.getExecutionEnvironment();
        streamExecutionEnvironment.addSource(new TestSourceFunction(new ExceptionThrowingAggregateFunction(), true)).sinkTo(new DiscardingSink<>());
        JobClient jobClient = streamExecutionEnvironment.executeAsync();
        Thread.sleep(1000);
        RestartFramework.at("during_global_aggregate").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        jobClient.getJobExecutionResult().get();
