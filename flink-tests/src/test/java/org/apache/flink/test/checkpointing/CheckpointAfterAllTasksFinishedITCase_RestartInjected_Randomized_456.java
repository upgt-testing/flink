 ...
 */
public class CheckpointAfterAllTasksFinishedITCase_RestartInjected_Randomized_456 {
    ...
    @RegisterExtension
    static final MiniClusterExtension MINI_CLUSTER_RESOURCE = new MiniClusterExtension(new MiniClusterResourceConfiguration.Builder().setNumberTaskManagers(1).setNumberSlotsPerTaskManager(4).withHaLeadershipControl().build());
    ...
    public void testImmediateCheckpointing() throws Exception {
        ...
        env.enableCheckpointing(Duration.ofNanos(Long.MAX_VALUE)./* max duration allowed by FLINK */
        toMillis());
    ...
    public void testRestoreAfterSomeTasksFinished() throws Exception {
        final MiniClusterConfiguration cfg = new MiniClusterConfiguration.Builder().withRandomPorts().setNumTaskManagers(1).setNumSlotsPerTaskManager(4).build();
        try (MiniCluster miniCluster = new MiniCluster(cfg)) {
            ...
            RestartFramework.at("after_first_job_submit").on(miniCluster).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Thread.sleep(500);
            RestartFramework.at("during_first_job_running").on(miniCluster).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            String savepointPath = miniCluster.triggerSavepoint(jobGraph.getJobID(), tmpDir.toFile().getAbsolutePath(), true, SavepointFormatType.CANONICAL).get();
            RestartFramework.at("after_savepoint").on(miniCluster).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            bigResult.get().clear();
            // Checkpointing is enabled with a large interval, and no checkpoints will be triggered.
            env.enableCheckpointing(Duration.ofNanos(Long.MAX_VALUE)./* max duration allowed by FLINK */
            toMillis());
            JobGraph restoredJobGraph = getStreamGraph(env, true, false).getJobGraph();
            restoredJobGraph.setSavepointRestoreSettings(SavepointRestoreSettings.forPath(savepointPath, false));
            miniCluster.submitJob(restoredJobGraph).get();
            RestartFramework.at("after_restore_job_submit").on(miniCluster).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testFailoverAfterSomeTasksFinished() throws Exception {
        ...
        final MiniClusterConfiguration cfg = new MiniClusterConfiguration.Builder().withRandomPorts().setNumTaskManagers(1).setNumSlotsPerTaskManager(4).setConfiguration(config).build();
        try (MiniCluster miniCluster = new MiniCluster(cfg)) {
            miniCluster.start();
            env.enableCheckpointing(100);
            JobGraph jobGraph = getStreamGraph(env, true, true).getJobGraph();
            miniCluster.submitJob(jobGraph).get();
            RestartFramework.at("after_job_submit").on(miniCluster).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Thread.sleep(500);
            RestartFramework.at("during_job_running").on(miniCluster).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            // records.
            assertThat(smallResult.get().size()).isIn(SMALL_SOURCE_NUM_RECORDS, SMALL_SOURCE_NUM_RECORDS * 2);
            assertThat(bigResult.get().size()).isEqualTo(BIG_SOURCE_NUM_RECORDS);
        }
    }

    private StreamGraph getStreamGraph(StreamExecutionEnvironment env, boolean block, boolean needFailover) {
        env.addSource(new IntegerStreamSource(SMALL_SOURCE_NUM_RECORDS, false, false)).transform("passA", Types.INT, new PassThroughOperator()).addSink(new CollectSink(smallResult)).name("sinkA");
        env.addSource(new IntegerStreamSource(BIG_SOURCE_NUM_RECORDS, block, needFailover)).transform("passB", Types.INT, new PassThroughOperator()).addSink(new CollectSink(bigResult)).name("sinkB");
    ...
    private static class PassThroughOperator extends AbstractStreamOperator<Integer> implements OneInputStreamOperator<Integer, Integer> {
