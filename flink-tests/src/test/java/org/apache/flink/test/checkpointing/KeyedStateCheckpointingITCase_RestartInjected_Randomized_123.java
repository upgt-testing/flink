...
public class KeyedStateCheckpointingITCase_RestartInjected_Randomized_123 extends TestLogger {
    ...
    // ------------------------------------------------------------------------
    @ClassRule
    public static final MiniClusterWithClientResource MINI_CLUSTER_RESOURCE = new MiniClusterWithClientResource(new MiniClusterResourceConfiguration.Builder().setConfiguration(getConfiguration()).setNumberTaskManagers(NUM_TASK_MANAGERS).setNumberSlotsPerTaskManager(NUM_TASK_SLOTS).build());
    ...
    public void testWithFsBackendAsync() throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        StateBackendUtils.configureHashMapStateBackend(env);
        CheckpointStorageUtils.configureFileSystemCheckpointStorage(env, tmpFolder.newFolder().toURI().toString());
        testProgramWithBackend(env);
    }
    ...
    public void testWithRocksDbBackendFull() throws Exception {
        ...
        env.configure(new Configuration().set(RocksDBOptions.LOCAL_DIRECTORIES, tmpFolder.newFolder().getAbsolutePath()));
        CheckpointStorageUtils.configureFileSystemCheckpointStorage(env, tmpFolder.newFolder().toURI().toString());
        testProgramWithBackend(env);
    }
    ...
    public void testWithForStBackendIncremental() throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.configure(new Configuration().set(StateBackendOptions.STATE_BACKEND, "forst").set(CheckpointingOptions.INCREMENTAL_CHECKPOINTS, true).set(ForStOptions.LOCAL_DIRECTORIES, tmpFolder.newFolder().getAbsolutePath()));
        CheckpointStorageUtils.configureFileSystemCheckpointStorage(env, tmpFolder.newFolder().toURI().toString());
        testProgramWithBackend(env);
    }
    ...
    protected void testProgramWithBackend(StreamExecutionEnvironment env) throws Exception {
        ...
        RestartFramework.at("during_keyed_state_processing").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        final DataStream<Integer> stream1 = env.addSource(new IntGeneratingSourceFunction(NUM_STRINGS / 2, NUM_STRINGS / 4));
        final DataStream<Integer> stream2 = env.addSource(new IntGeneratingSourceFunction(NUM_STRINGS / 2, NUM_STRINGS / 4));
        stream1.union(stream2).keyBy(new IdentityKeySelector<Integer>()).map(new OnceFailingPartitionedSum(failurePos)).keyBy(x -> x.f0).addSink(new CounterSink());
        ...
        // parallel operator
        assertEquals(recoveryCounterBeforeTest + PARALLELISM, OnceFailingPartitionedSum.RECOVERY_COUNTER.get());
    ...
    private static class IntGeneratingSourceFunction extends RichParallelSourceFunction<Integer> implements ListCheckpointed<Integer>, CheckpointListener {
        ...
        public void run(SourceContext<Integer> ctx) throws Exception {
            ...
            int nextElement = lastEmitted >= 0 ? lastEmitted + step : getRuntimeContext().getTaskInfo().getIndexOfThisSubtask();
