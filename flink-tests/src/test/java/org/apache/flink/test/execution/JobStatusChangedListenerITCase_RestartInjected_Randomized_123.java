...
public class JobStatusChangedListenerITCase_RestartInjected_Randomized_123 extends TestLogger {
    ...
    @ClassRule
    public static final MiniClusterWithClientResource MINI_CLUSTER = new MiniClusterWithClientResource(new MiniClusterResourceConfiguration.Builder().setConfiguration(createConfiguration()).setNumberTaskManagers(1).setNumberSlotsPerTaskManager(PARALLELISM).withHaLeadershipControl().build());
    ...
    public void testJobStatusChangedForSucceededApplication() throws Exception {
        ...
        try (StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment(configuration)) {
            ...
            try (CloseableIterator<String> iterator = env.fromCollection(sourceValues).executeAndCollect()) {
                RestartFramework.at("after_job_submit").on(MINI_CLUSTER).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        statusChangedEvents.forEach(event -> {
            if (event instanceof DefaultJobExecutionStatusEvent) {
                ...
                assertThat((status.oldStatus() == JobStatus.CREATED && status.newStatus() == JobStatus.RUNNING) || (status.oldStatus() == JobStatus.RUNNING && status.newStatus() == JobStatus.FINISHED)).isTrue();
            } else {
                DefaultJobCreatedEvent createdEvent = (DefaultJobCreatedEvent) event;
                assertThat(createdEvent.executionMode()).isEqualTo(RuntimeExecutionMode.STREAMING);
            }
        });
    }
    ...
    public void testJobStatusChangedForFailedApplication() throws Exception {
        ...
        statusChangedEvents.forEach(event -> {
            JobExecutionStatusEvent status = (JobExecutionStatusEvent) event;
            assertThat((status.oldStatus() == JobStatus.CREATED && status.newStatus() == JobStatus.RUNNING) || (status.oldStatus() == JobStatus.RUNNING && status.newStatus() == JobStatus.FAILING) || (status.oldStatus() == JobStatus.FAILING && status.newStatus() == JobStatus.FAILED)).isTrue();
        });
    }
    ...
    public void testJobStatusChangedForCancelledApplication() throws Exception {
        ...
        try (StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment(configuration)) {
            ...
            RestartFramework.at("during_job_running").on(MINI_CLUSTER).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Thread.sleep(100);
        ...
        statusChangedEvents.forEach(event -> {
            JobExecutionStatusEvent status = (JobExecutionStatusEvent) event;
            assertThat((status.oldStatus() == JobStatus.CREATED && status.newStatus() == JobStatus.RUNNING) || (status.oldStatus() == JobStatus.RUNNING && status.newStatus() == JobStatus.CANCELLING) || (status.oldStatus() == JobStatus.CANCELLING && status.newStatus() == JobStatus.CANCELED)).isTrue();
    ...
    void verifyLineageGraph(LineageGraph lineageGraph) {
        assertThat(lineageGraph.sources().size()).isEqualTo(1);
        assertThat(lineageGraph.sources().get(0).boundedness()).isEqualTo(Boundedness.CONTINUOUS_UNBOUNDED);
        assertThat(lineageGraph.sources().get(0).datasets().size()).isEqualTo(1);
        assertThat(lineageGraph.sources().get(0).datasets().get(0).name()).isEqualTo(SOURCE_DATASET_NAME);
        assertThat(lineageGraph.sources().get(0).datasets().get(0).namespace()).isEqualTo(SOURCE_DATASET_NAMESPACE);
        assertThat(lineageGraph.sinks().size()).isEqualTo(1);
        assertThat(lineageGraph.sinks().get(0).datasets().size()).isEqualTo(1);
        assertThat(lineageGraph.sinks().get(0).datasets().get(0).name()).isEqualTo(SINK_DATASET_NAME);
    ...
    void verifyEventMetaData() {
        ...
        assertThat(statusChangedEvents.get(0).jobId()).isEqualTo(statusChangedEvents.get(1).jobId());
        assertThat(statusChangedEvents.get(0).jobName()).isEqualTo(statusChangedEvents.get(1).jobName());
        assertThat(statusChangedEvents.get(1).jobId()).isEqualTo(statusChangedEvents.get(2).jobId());
        assertThat(statusChangedEvents.get(1).jobName()).isEqualTo(statusChangedEvents.get(2).jobName());
    }

    /**
     * Testing job status changed listener factory.
     */
    public static class TestingJobStatusChangedListenerFactory implements JobStatusChangedListenerFactory {
    ...
    private static Configuration createConfiguration() {
        ...
        configuration.set(JOB_STATUS_CHANGED_LISTENERS, Collections.singletonList(TestingJobStatusChangedListenerFactory.class.getName()));
    ...
    private static class InfiniteLongSourceFunction implements SourceFunction<Long>, LineageVertexProvider {
        ...
        public LineageVertex getLineageVertex() {
            LineageDataset lineageDataset = new DefaultLineageDataset(SOURCE_DATASET_NAME, SOURCE_DATASET_NAMESPACE, new HashMap<>());
    ...
    private static class SleepingSink implements SinkFunction<Long>, LineageVertexProvider {
        ...
        @Override
        public LineageVertex getLineageVertex() {
            LineageDataset lineageDataset = new DefaultLineageDataset(SINK_DATASET_NAME, SINK_DATASET_NAMESPACE, new HashMap<>());
