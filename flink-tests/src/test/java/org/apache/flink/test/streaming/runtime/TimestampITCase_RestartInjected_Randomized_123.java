...
public class TimestampITCase_RestartInjected_Randomized_123 extends TestLogger {
    ...
    @ClassRule
    public static final MiniClusterWithClientResource CLUSTER = new MiniClusterWithClientResource(new MiniClusterResourceConfiguration.Builder().setConfiguration(getConfiguration()).setNumberTaskManagers(NUM_TASK_MANAGERS).setNumberSlotsPerTaskManager(NUM_TASK_SLOTS).withHaLeadershipControl().build());
    ...
    public void testWatermarkPropagation() throws Exception {
        RestartFramework.at("during_watermark_propagation").on(CLUSTER).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        final int numWatermarks = 10;
        long initialTime = 0L;
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.setParallelism(PARALLELISM);
        RestartStrategyUtils.configureFixedDelayRestartStrategy(env, 5, 1000L);
        RestartFramework.at("after_job_submit").on(CLUSTER).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        source1.union(source2).map(new IdentityMap()).connect(source2).map(new IdentityCoMap()).transform("Custom Operator", BasicTypeInfo.INT_TYPE_INFO, new CustomOperator(true)).sinkTo(new DiscardingSink<>());
        org.apache.flink.core.execution.JobClient jobClient = env.executeAsync();
        Thread.sleep(500);
    ...
    public void testSelfUnionWatermarkPropagation() throws Exception {
        ...
        dataStream1.union(dataStream1).transform("Custom Operator", BasicTypeInfo.INT_TYPE_INFO, new CustomOperator(false)).sinkTo(new DiscardingSink<>());
        org.apache.flink.core.execution.JobClient jobClient = env.executeAsync();
        jobClient.getJobExecutionResult().get();
        RestartFramework.at("after_job_submit").on(CLUSTER).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testWatermarkPropagationNoFinalWatermarkOnStop() throws Exception {
        ...
        source1.union(source2).map(new IdentityMap()).connect(source2).map(new IdentityCoMap()).transform("Custom Operator", BasicTypeInfo.INT_TYPE_INFO, new CustomOperator(true)).sinkTo(new DiscardingSink<Integer>());
        Thread t = new Thread("stopper") {
            ...
            public void run() {
                try {
                    ...
                    do {
                        try {
                            clusterClient.stopWithSavepoint(id, false, savepointDirName, SavepointFormatType.CANONICAL).get();
                        } catch (Exception e) {
                            boolean ignoreException = ExceptionUtils.findThrowable(e, CheckpointException.class).map(CheckpointException::getCheckpointFailureReason).map(reason -> reason == CheckpointFailureReason.NOT_ALL_REQUIRED_TASKS_RUNNING).orElse(false);
                            if (!ignoreException) {
                                throw e;
                            }
                        }
    ...
    public void testTimestampHandling() throws Exception {
        ...
        source1.map(new IdentityMap()).connect(source2).map(new IdentityCoMap()).transform("Custom Operator", BasicTypeInfo.INT_TYPE_INFO, new TimestampCheckingOperator()).sinkTo(new DiscardingSink<Integer>());
        org.apache.flink.core.execution.JobClient jobClient = env.executeAsync();
        RestartFramework.at("after_job_submit").on(CLUSTER).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        jobClient.getJobExecutionResult().get();
    }
    ...
    public void testDisabledTimestamps() throws Exception {
        ...
        source1.map(new IdentityMap()).connect(source2).map(new IdentityCoMap()).transform("Custom Operator", BasicTypeInfo.INT_TYPE_INFO, new DisabledTimestampCheckingOperator()).sinkTo(new DiscardingSink<Integer>());
        org.apache.flink.core.execution.JobClient jobClient = env.executeAsync();
        jobClient.getJobExecutionResult().get();
    }
    ...
    public void testTimestampExtractorWithAutoInterval() throws Exception {
        ...
        DataStream<Integer> extractOp = source1.assignTimestampsAndWatermarks(AscendingRecordTimestampsWatermarkStrategy.create(Long::valueOf));
    ...
    public void testTimestampExtractorWithCustomWatermarkEmit() throws Exception {
        ...
        source1.assignTimestampsAndWatermarks(new WatermarkStrategy<Integer>() {
    ...
    public void testTimestampExtractorWithLongMaxWatermarkFromSource() throws Exception {
        ...
        source1.assignTimestampsAndWatermarks((WatermarkStrategy<Integer>) context -> new NoWatermarksGenerator<>()).transform("Watermark Check", BasicTypeInfo.INT_TYPE_INFO, new CustomOperator(true));
        env.execute();
    ...
    public void testErrorOnEventTimeOverProcessingTime() {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.setParallelism(2);
        DataStream<Tuple2<String, Integer>> source1 = env.fromData(new Tuple2<>("a", 1), new Tuple2<>("b", 2));
        source1.keyBy(x -> x.f0).window(TumblingEventTimeWindows.of(Duration.ofSeconds(5))).reduce(new ReduceFunction<Tuple2<String, Integer>>() {
    ...
    public static class AscendingRecordTimestampsWatermarkStrategy<T> implements WatermarkStrategy<T> {

        private final SerializableFunction<T, Long> timestampExtractor;

        public AscendingRecordTimestampsWatermarkStrategy(SerializableFunction<T, Long> timestampExtractor) {
            this.timestampExtractor = timestampExtractor;
        }

        public static <T> AscendingRecordTimestampsWatermarkStrategy<T> create(SerializableFunction<T, Long> timestampExtractor) {
