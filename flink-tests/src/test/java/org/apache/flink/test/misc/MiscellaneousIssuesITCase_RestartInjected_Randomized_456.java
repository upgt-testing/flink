...
public class MiscellaneousIssuesITCase_RestartInjected_Randomized_456 extends TestLogger {
    ...
    public void testNullValues() throws Exception {
        ...
        DataStream<String> data = env.fromData("hallo").map(new MapFunction<String, String>() {
        ...
        data.sinkTo(FileSink.forRowFormat(new Path("/tmp/myTest"), new SimpleStringEncoder<String>()).build());
        try {
            env.execute();
    ...
    public void testDisjointDataflows() throws Exception {
        ...
        env.fromSequence(1, 10).sinkTo(new DiscardingSink<>());
        JobClient jobClient = env.executeAsync("Disjoint Dataflows Test");
    ...
    public void testAccumulatorsAfterNoOp() throws Exception {
        ...
        env.fromSequence(1, 1000000).rebalance().flatMap(new RichFlatMapFunction<Long, Long>() {
        ...
        JobClient jobClient = env.executeAsync("Accumulators After NoOp Test");
        RestartFramework.at("during_job_running").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_job_submit").on(MINI_CLUSTER_RESOURCE).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(2000);
