 ...
 */
public class BufferTimeoutITCase_RestartInjected_Randomized_123 extends AbstractTestBaseJUnit4 {
    ...
    public void testDisablingBufferTimeout() throws Exception {
        ...
        RestartFramework.at("during_buffer_timeout_test").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        env.setBufferTimeout(-1);
        final SharedReference<ArrayList<Integer>> results = sharedObjects.add(new ArrayList<>());
        env.fromSource(createSingleElementIdleSource(), WatermarkStrategy.<Integer>noWatermarks(), "v2-src").slotSharingGroup("source").sinkTo(createResultCollectingSink(results)).slotSharingGroup("sink");
     ...
     */
    private static Sink<Integer> createResultCollectingSink(SharedReference<ArrayList<Integer>> results) {
        return context -> new SinkWriter<>() {
