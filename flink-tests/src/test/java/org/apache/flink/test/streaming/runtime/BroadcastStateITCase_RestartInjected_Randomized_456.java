 ...
 */
public class BroadcastStateITCase_RestartInjected_Randomized_456 extends AbstractTestBaseJUnit4 {
    ...
    public void testKeyedWithBroadcastTranslation() throws Exception {
        ...
        RestartFramework.at("during_broadcast_processing").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        final DataStream<Long> srcOne = env.fromSequence(0L, 5L).assignTimestampsAndWatermarks(new CustomWmEmitter<Long>() {
        ...
        final DataStream<String> srcTwo = env.fromData(expected.values()).assignTimestampsAndWatermarks(new CustomWmEmitter<String>() {
        ...
        JobClient jobClient = env.executeAsync("Keyed Broadcast Test");
        Thread.sleep(500);
    ...
    public void testBroadcastTranslation() throws Exception {
        ...
        JobClient jobClient = env.executeAsync("Broadcast Test");
        RestartFramework.at("after_job_submit").on(MINI_CLUSTER_RESOURCE).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(500);
        jobClient.getJobExecutionResult().get();
    }
    ...
    private abstract static class CustomWmEmitter<T> implements WatermarkStrategyWithPunctuatedWatermarks<T> {
