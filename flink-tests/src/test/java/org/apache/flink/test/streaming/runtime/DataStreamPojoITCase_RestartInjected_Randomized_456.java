 ...
 * <p>See FLINK-3697
 */
public class DataStreamPojoITCase_RestartInjected_Randomized_456 extends AbstractTestBaseJUnit4 {
    ...
    public void testCompositeKeyOnNestedPojo() throws Exception {
        ...
        DataStream<Data> summedStream = dataStream.keyBy(x -> Tuple3.of(x.aaa, x.abc, x.wxyz), Types.TUPLE(Types.INT, Types.INT, Types.LONG)).sum("sum").keyBy(x -> Tuple3.of(x.aaa, x.abc, x.wxyz), Types.TUPLE(Types.INT, Types.INT, Types.LONG)).flatMap(new FlatMapFunction<Data, Data>() {
        ...
        summedStream.print();
        JobClient jobClient = see.executeAsync();
        ...
        Thread.sleep(1000);
        RestartFramework.at("during_keyed_processing").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testNestedKeyOnNestedPojo() throws Exception {
        ...
        DataStream<Data> dataStream = see.fromData(elements);
        RestartFramework.at("after_job_submit").on(MINI_CLUSTER_RESOURCE).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DataStream<Data> summedStream = dataStream.keyBy(x -> Tuple2.of(x.aaa, x.stats.count), Types.TUPLE(Types.INT, Types.LONG)).sum("sum").keyBy(x -> Tuple2.of(x.aaa, x.stats.count), Types.TUPLE(Types.INT, Types.LONG)).flatMap(new FlatMapFunction<Data, Data>() {
    ...
    public void testNestedPojoFieldAccessor() throws Exception {
        ...
        see.setParallelism(4);
        DataStream<Data> dataStream = see.fromData(elements);
        DataStream<Data> summedStream = dataStream.keyBy(x -> x.aaa).sum("stats.count").keyBy(x -> x.aaa).flatMap(new FlatMapFunction<Data, Data>() {
    ...
    public void testFailOnNestedPojoFieldAccessor() throws Exception {
        StreamExecutionEnvironment see = StreamExecutionEnvironment.getExecutionEnvironment();
        DataStream<Data> dataStream = see.fromData(elements);
        dataStream.keyBy(x -> Tuple2.of(x.aaa, x.stats.count), Types.TUPLE(Types.INT, Types.LONG)).sum("stats.nonExistingField");
    }
