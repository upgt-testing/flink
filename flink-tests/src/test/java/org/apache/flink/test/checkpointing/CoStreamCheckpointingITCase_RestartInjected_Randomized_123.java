...
public class CoStreamCheckpointingITCase_RestartInjected_Randomized_123 extends AbstractTestBaseJUnit4 {
    ...
    public void testCoStreamCheckpointingProgram() throws Exception {
        ...
        stream.// -------------- first vertex, chained to the source ----------------
        filter(new StringRichFilterFunction()).// -------------- second vertex - stateful ----------------
        connect(stream).flatMap(new LeftIdentityCoRichFlatMapFunction()).// -------------- third vertex - stateful  ----------------
        map(new StringPrefixCountRichMapFunction()).startNewChain().map(new StatefulCounterFunction()).// -------------- fourth vertex - reducer (failing) and the sink ----------------
        keyBy(x -> x.prefix).reduce(new OnceFailingReducer(NUM_STRINGS)).addSink(new SinkFunction<PrefixCount>() {
        ...
        graph.setJobName("Fault Tolerance Test");
        JobClient jobClient = env.executeAsync(graph);
        Thread.sleep(200);
        Thread.sleep(300);
        RestartFramework.at("during_comap_execution").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
