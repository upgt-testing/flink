...
public class ProcessingTimeWindowCheckpointingITCase_RestartInjected_Randomized_123 extends TestLogger {

    private static final int PARALLELISM = 4;

    @ClassRule
    public static final MiniClusterWithClientResource MINI_CLUSTER_RESOURCE = new MiniClusterWithClientResource(new MiniClusterResourceConfiguration.Builder().setConfiguration(getConfiguration()).setNumberTaskManagers(2).setNumberSlotsPerTaskManager(PARALLELISM / 2).build());
    ...
    public void testTumblingProcessingTimeWindow() throws Exception {
        ...
        RestartFramework.at("after_checkpoint").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        env.addSource(new FailingSource(new Generator(), numElements, true)).rebalance().keyBy(x -> x.f0).window(TumblingProcessingTimeWindows.of(Duration.ofMillis(100))).apply(new RichWindowFunction<Tuple2<Long, IntType>, Tuple2<Long, IntType>, Long, TimeWindow>() {
            ...
            @Override
            public void apply(Long l, TimeWindow window, Iterable<Tuple2<Long, IntType>> values, Collector<Tuple2<Long, IntType>> out) {
    ...
    public void testSlidingProcessingTimeWindow() throws Exception {
        ...
        env.addSource(new FailingSource(new Generator(), numElements, true)).rebalance().keyBy(x -> x.f0).window(SlidingProcessingTimeWindows.of(Duration.ofMillis(150), Duration.ofMillis(50))).apply(new RichWindowFunction<Tuple2<Long, IntType>, Tuple2<Long, IntType>, Long, TimeWindow>() {
        ...
        JobClient jobClient = env.executeAsync("Sliding Window Test");
        Thread.sleep(200);
        Thread.sleep(300);
        RestartFramework.at("during_window_processing").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        jobClient.getJobExecutionResult().get();
    ...
    public void testAggregatingTumblingProcessingTimeWindow() throws Exception {
        ...
        }).rebalance().keyBy(x -> x.f0).window(TumblingProcessingTimeWindows.of(Duration.ofMillis(100))).reduce(new ReduceFunction<Tuple2<Long, IntType>>() {
        ...
        JobClient jobClient = env.executeAsync("Aggregating Tumbling Window Test");
        RestartFramework.at("during_window_processing").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(200);
        Thread.sleep(300);
        jobClient.getJobExecutionResult().get();
    ...
    public void testAggregatingSlidingProcessingTimeWindow() throws Exception {
        ...
        }).rebalance().keyBy(x -> x.f0).window(SlidingProcessingTimeWindows.of(Duration.ofMillis(150), Duration.ofMillis(50))).reduce(new ReduceFunction<Tuple2<Long, IntType>>() {
        ...
        JobClient jobClient = env.executeAsync("Aggregating Sliding Window Test");
        Thread.sleep(200);
        Thread.sleep(300);
        RestartFramework.at("during_window_processing").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_checkpoint").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        public boolean checkResult(Map<Long, Integer> windowCounts) {
            ...
            for (Map.Entry<Long, Integer> e : windowCounts.entrySet()) {
                if (e.getValue() < countPerElementExpected) {
                    ...
                    fail(String.format("counter too big for %d: %d (expected %d)", e.getKey(), e.getValue(), countPerElementExpected));
                }
            }
            return true;
        }
