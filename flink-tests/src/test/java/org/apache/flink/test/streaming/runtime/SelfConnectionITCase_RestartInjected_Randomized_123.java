...
public class SelfConnectionITCase_RestartInjected_Randomized_123 extends AbstractTestBaseJUnit4 {
    ...
    public void differentDataStreamSameChain() throws Exception {
        ...
        JobClient jobClient = env.executeAsync("Self Connection Same Chain Test");
        Thread.sleep(1000);
    ...
    public void differentDataStreamDifferentChain() throws Exception {
        ...
        JobClient jobClient = env.executeAsync("Self Connection Different Chain Test");
        Thread.sleep(1000);
        jobClient.getJobExecutionResult().get();
        RestartFramework.at("during_job_running").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        List<String> expected = Arrays.asList("x 1", "x 3", "x 5", "2", "4", "6");
