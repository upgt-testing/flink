 ...
 */
public class SinkV2MetricsITCase_RestartInjected_Randomized_456 extends TestLogger {
    ...
    @Rule
    public final MiniClusterWithClientResource miniClusterResource = new MiniClusterWithClientResource(new MiniClusterResourceConfiguration.Builder().setNumberTaskManagers(1).setNumberSlotsPerTaskManager(DEFAULT_PARALLELISM).setConfiguration(reporter.addToConfiguration(new Configuration())).withHaLeadershipControl().build());

    @Test
    public void testMetrics() throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        int numSplits = Math.max(1, env.getParallelism() - 2);
        RestartFramework.at("after_job_submit").on(miniClusterResource).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("during_sink_metrics_check").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        int stopAtRecord1 = 4;
        ...
        env.fromSequence(0, numSplits - 1).<Long>flatMap((split, collector) -> LongStream.range(0, numRecordsPerSplit).forEach(collector::collect)).returns(BasicTypeInfo.LONG_TYPE_INFO).map(i -> {
            ...
            return i;
        }).sinkTo(TestSinkV2.<Long>newBuilder().setWriter(new MetricWriter()).build()).name(TEST_SINK_NAME);
    ...
    public void testCommitterMetrics() throws Exception {
        ...
        // metrics
        SharedReference<CountDownLatch> beforeLatch = sharedObjects.add(new CountDownLatch(numCommittables));
        SharedReference<CountDownLatch> afterLatch = sharedObjects.add(new CountDownLatch(1));
        RestartFramework.at("during_committer_execution").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        TestSinkV2<Long> sink = TestSinkV2.<Long>newBuilder().setCommitter(new MetricCommitter(beforeLatch, afterLatch), TestSinkV2.RecordSerializer::new).build();
        env.fromSequence(0, numCommittables - 1).returns(BasicTypeInfo.LONG_TYPE_INFO).sinkTo(sink).name(TEST_SINK_NAME);
        JobClient jobClient = env.executeAsync();
        final JobID jobId = jobClient.getJobID();
        RestartFramework.at("after_committer_job_submit").on(miniClusterResource).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // Run until every committer finished with 1 commit round - everything should be retried and
        // pending
        beforeLatch.get().await();
        assertSinkCommitterMetrics(jobId, ImmutableMap.of(MetricNames.ALREADY_COMMITTED_COMMITTABLES, 0L, MetricNames.FAILED_COMMITTABLES, 0L, MetricNames.RETRIED_COMMITTABLES, 7L, MetricNames.SUCCESSFUL_COMMITTABLES, 0L, MetricNames.TOTAL_COMMITTABLES, 7L, MetricNames.PENDING_COMMITTABLES, 7L));
        afterLatch.get().countDown();
        // Run until finished
        jobClient.getJobExecutionResult().get();
        assertSinkCommitterMetrics(jobId, ImmutableMap.of(MetricNames.ALREADY_COMMITTED_COMMITTABLES, 1L, MetricNames.FAILED_COMMITTABLES, 2L, MetricNames.RETRIED_COMMITTABLES, 10L, MetricNames.SUCCESSFUL_COMMITTABLES, 4L, MetricNames.TOTAL_COMMITTABLES, 7L, MetricNames.PENDING_COMMITTABLES, 0L));
    }

    @SuppressWarnings("checkstyle:WhitespaceAfter")
    private void assertSinkMetrics(JobID jobId, long processedRecordsPerSubtask, int numSplits) {
        List<OperatorMetricGroup> groups = reporter.findOperatorMetricGroups(jobId, TEST_SINK_NAME + ": " + DEFAULT_WRITER_NAME);
        ...
        for (OperatorMetricGroup group : groups) {
            ...
            if (group.getIOMetricGroup() == null || group.getIOMetricGroup().getNumRecordsOutCounter() == null || group.getIOMetricGroup().getNumRecordsOutCounter().getCount() == 0) {
                continue;
            }
            subtaskWithMetrics++;
            // SinkWriterMetricGroup metrics
            assertThatCounter(metrics.get(MetricNames.IO_NUM_RECORDS_OUT)).isEqualTo(processedRecordsPerSubtask);
            assertThatCounter(metrics.get(MetricNames.IO_NUM_BYTES_OUT)).isEqualTo(processedRecordsPerSubtask * MetricWriter.RECORD_SIZE_IN_BYTES);
            // MetricWriter is just incrementing errors every even record
            assertThatCounter(metrics.get(MetricNames.NUM_RECORDS_OUT_ERRORS)).isEqualTo((processedRecordsPerSubtask + 1) / 2);
            // Test "send" metric series has the same value as "out" metric series.
            assertThatCounter(metrics.get(MetricNames.NUM_RECORDS_SEND)).isEqualTo(processedRecordsPerSubtask);
            assertThatCounter(metrics.get(MetricNames.NUM_BYTES_SEND)).isEqualTo(processedRecordsPerSubtask * MetricWriter.RECORD_SIZE_IN_BYTES);
            assertThatCounter(metrics.get(MetricNames.NUM_RECORDS_SEND_ERRORS)).isEqualTo((processedRecordsPerSubtask + 1) / 2);
            // check if the latest send time is fetched
            assertThatGauge(metrics.get(MetricNames.CURRENT_SEND_TIME)).isEqualTo((processedRecordsPerSubtask - 1) * MetricWriter.BASE_SEND_TIME);
        }
        assertThat(subtaskWithMetrics, equalTo(numSplits));
        // Test operator I/O metrics are reused by task metrics
        List<TaskMetricGroup> taskMetricGroups = reporter.findTaskMetricGroups(jobId, TEST_SINK_NAME);
        ...
        for (TaskMetricGroup taskMetricGroup : taskMetricGroups) {
            ...
            assertThatCounter(taskMetricGroup.getIOMetricGroup().getNumRecordsOutCounter()).isEqualTo(processedRecordsPerSubtask);
            assertThatCounter(taskMetricGroup.getIOMetricGroup().getNumBytesOutCounter()).isEqualTo(processedRecordsPerSubtask * MetricWriter.RECORD_SIZE_IN_BYTES);
        }
    ...
    private void assertSinkCommitterMetrics(JobID jobId, Map<String, Long> expected) {
        List<OperatorMetricGroup> groups = reporter.findOperatorMetricGroups(jobId, TEST_SINK_NAME + ": " + DEFAULT_COMMITTER_NAME);
        Map<String, Long> aggregated = new HashMap<>(6);
        for (OperatorMetricGroup group : groups) {
            Map<String, Metric> metrics = reporter.getMetricsByGroup(group);
            for (String metricName : Arrays.asList(MetricNames.SUCCESSFUL_COMMITTABLES, MetricNames.ALREADY_COMMITTED_COMMITTABLES, MetricNames.RETRIED_COMMITTABLES, MetricNames.FAILED_COMMITTABLES, MetricNames.TOTAL_COMMITTABLES)) {
                final Counter counter = (Counter) metrics.get(metricName);
                if (counter != null) {
                    aggregated.merge(metricName, counter.getCount(), Long::sum);
                }
            }
            Gauge<Integer> pendingMetrics = (Gauge<Integer>) metrics.get(MetricNames.PENDING_COMMITTABLES);
            if (pendingMetrics != null && pendingMetrics.getValue() != null) {
                aggregated.merge(MetricNames.PENDING_COMMITTABLES, pendingMetrics.getValue().longValue(), Long::sum);
            }
        }
        expected.entrySet().forEach(e -> assertThat(aggregated, hasEntry(e.getKey(), e.getValue())));
    }
    ...
    private static class MetricCommitter extends TestSinkV2.DefaultCommitter<Record<Long>> {
        ...
        public void commit(Collection<CommitRequest<Record<Long>>> committables) {
            if (counter == 0) {
                System.err.println("Committables arrived " + Thread.currentThread().getName() + " " + committables.stream().map(c -> c.getCommittable()).collect(Collectors.toList()));
