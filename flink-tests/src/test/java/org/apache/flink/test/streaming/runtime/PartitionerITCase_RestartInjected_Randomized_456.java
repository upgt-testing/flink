...
public class PartitionerITCase_RestartInjected_Randomized_456 extends AbstractTestBaseJUnit4 {
    ...
    public void partitionerTest() throws Exception {
        ...
        DataStream<Tuple1<String>> src = env.fromData(INPUT.stream().map(Tuple1::of).collect(Collectors.toList()));
        // partition by hash
        ...
        }, x -> x.f0).map(new SubtaskIndexAssigner());
        ...
        JobClient jobClient = env.executeAsync("Partitioner Test");
        Thread.sleep(1000);
    ...
    private static void verifyBroadcastPartitioning(List<Tuple2<Integer, String>> broadcastPartitionResult) {
        final Set<Tuple2<Integer, String>> expectedResult = INPUT.stream().flatMap(input -> IntStream.range(0, PARALLELISM).mapToObj(i -> Tuple2.of(i, input))).collect(Collectors.toSet());
    ...
    private static void verifyRebalancePartitioning(List<Tuple2<Integer, String>> rebalancePartitionResult) {
        ...
        final List<Tuple2<Integer, String>> expected = IntStream.range(0, rebalancePartitionResult.size()).mapToObj(index -> Tuple2.of((offset + index) % PARALLELISM, INPUT.get(index))).collect(Collectors.toList());
