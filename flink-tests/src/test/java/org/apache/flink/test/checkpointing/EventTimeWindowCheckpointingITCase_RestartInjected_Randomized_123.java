...
import static org.apache.flink.test.checkpointing.EventTimeWindowCheckpointingITCase_RestartInjected.StateBackendEnum.ROCKSDB_INCREMENTAL_ZK;
...
public class EventTimeWindowCheckpointingITCase_RestartInjected_Randomized_123 extends TestLogger {
    ...
    public static Collection<Object[]> parameter() {
        return Arrays.stream(StateBackendEnum.values()).map((type) -> new Object[][] { { type } }).flatMap(Arrays::stream).collect(Collectors.toList());
    }

    public EventTimeWindowCheckpointingITCase_RestartInjected_Randomized_123(StateBackendEnum stateBackendEnum) {
    ...
    protected final MiniClusterWithClientResource getMiniClusterResource() {
        return new MiniClusterWithClientResource(new MiniClusterResourceConfiguration.Builder().setConfiguration(configuration).setNumberTaskManagers(NUM_OF_TASK_MANAGERS).setNumberSlotsPerTaskManager(PARALLELISM / NUM_OF_TASK_MANAGERS).build());
    }
    ...
    private Configuration getConfiguration() throws Exception {
        ...
        switch(stateBackendEnum) {
            ...
            case FILE:
                {
                    ...
                    config.set(StateBackendOptions.STATE_BACKEND, "hashmap");
                    config.set(CheckpointingOptions.CHECKPOINTS_DIRECTORY, Path.fromLocalFile(backups).toUri().toString());
                    break;
                }
            case ROCKSDB_FULL:
                {
            ...
            case FORST_INCREMENTAL:
                {
                    config.set(ForStOptions.TIMER_SERVICE_FACTORY, ForStStateBackend.PriorityQueueStateType.ForStDB);
        ...
        FsStateChangelogStorageFactory.configure(config, tempFolder.newFolder(), Duration.ofMinutes(1), 10);
        return config;
    }
    ...
    private void setupRocksDB(Configuration config, int fileSizeThreshold, boolean incrementalCheckpoints) throws IOException {
        ...
        if (fileSizeThreshold != -1) {
            config.set(CheckpointingOptions.FS_SMALL_FILE_THRESHOLD, MemorySize.parse(fileSizeThreshold + "b"));
        }
    ...
    public void testTumblingTimeWindow() throws Exception {
        ...
        RestartFramework.at("after_checkpoint").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment(configuration);
        env.setParallelism(PARALLELISM);
        env.enableCheckpointing(100);
        RestartStrategyUtils.configureFixedDelayRestartStrategy(env, 5, 1000L);
        env.getConfig().setUseSnapshotCompression(true);
        env.addSource(new FailingSource(new KeyedEventTimeGenerator(numKeys, windowSize), numElementsPerKey)).rebalance().keyBy(x -> x.f0).window(TumblingEventTimeWindows.of(Duration.ofMillis(windowSize))).apply(new RichWindowFunction<Tuple2<Long, IntType>, Tuple4<Long, Long, Long, IntType>, Long, TimeWindow>() {

            private boolean open = false;

            @Override
            public void open(OpenContext openContext) {
                assertEquals(PARALLELISM, getRuntimeContext().getTaskInfo().getNumberOfParallelSubtasks());
                open = true;
            }

            @Override
            public void apply(Long l, TimeWindow window, Iterable<Tuple2<Long, IntType>> values, Collector<Tuple4<Long, Long, Long, IntType>> out) {
        ...
        JobClient jobClient = env.executeAsync("Tumbling Window Test");
        RestartFramework.at("during_window_processing").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Thread.sleep(200);
        Thread.sleep(300);
    ...
    public void doTestTumblingTimeWindowWithKVState(int maxParallelism) throws Exception {
        ...
        env.addSource(new FailingSource(new KeyedEventTimeGenerator(numKeys, windowSize), numElementsPerKey)).rebalance().keyBy(x -> x.f0).window(TumblingEventTimeWindows.of(Duration.ofMillis(windowSize))).apply(new RichWindowFunction<Tuple2<Long, IntType>, Tuple4<Long, Long, Long, IntType>, Long, TimeWindow>() {
            ...
            @Override
            public void apply(Long l, TimeWindow window, Iterable<Tuple2<Long, IntType>> values, Collector<Tuple4<Long, Long, Long, IntType>> out) throws Exception {
                ...
                out.collect(new Tuple4<>(l, window.getStart(), window.getEnd(), new IntType(count.value())));
            }
    ...
    public void testSlidingTimeWindow() throws Exception {
        ...
        RestartStrategyUtils.configureFixedDelayRestartStrategy(env, 5, 1000L);
        env.getConfig().setUseSnapshotCompression(true);
        env.addSource(new FailingSource(new KeyedEventTimeGenerator(numKeys, windowSlide), numElementsPerKey)).rebalance().keyBy(x -> x.f0).window(SlidingEventTimeWindows.of(Duration.ofMillis(windowSize), Duration.ofMillis(windowSlide))).apply(new RichWindowFunction<Tuple2<Long, IntType>, Tuple4<Long, Long, Long, IntType>, Long, TimeWindow>() {
    ...
    public void testPreAggregatedTumblingTimeWindow() throws Exception {
        ...
        env.getConfig().setUseSnapshotCompression(true);
        RestartFramework.at("after_job_submit").on(miniClusterResource).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        env.addSource(new FailingSource(new KeyedEventTimeGenerator(numKeys, windowSize), numElementsPerKey)).rebalance().keyBy(x -> x.f0).window(TumblingEventTimeWindows.of(Duration.ofMillis(windowSize))).reduce(new ReduceFunction<Tuple2<Long, IntType>>() {

            @Override
            public Tuple2<Long, IntType> reduce(Tuple2<Long, IntType> a, Tuple2<Long, IntType> b) {
                return new Tuple2<>(a.f0, new IntType(a.f1.value + b.f1.value));
            }
        }, new RichWindowFunction<Tuple2<Long, IntType>, Tuple4<Long, Long, Long, IntType>, Long, TimeWindow>() {
            ...
            @Override
            public void apply(Long l, TimeWindow window, Iterable<Tuple2<Long, IntType>> input, Collector<Tuple4<Long, Long, Long, IntType>> out) {
                // validate that the function has been opened properly
                assertTrue(open);
    ...
    public void testPreAggregatedSlidingTimeWindow() throws Exception {
        ...
        RestartStrategyUtils.configureFixedDelayRestartStrategy(env, 5, 1000L);
        env.getConfig().setUseSnapshotCompression(true);
        env.addSource(new FailingSource(new KeyedEventTimeGenerator(numKeys, windowSlide), numElementsPerKey)).rebalance().keyBy(x -> x.f0).window(SlidingEventTimeWindows.of(Duration.ofMillis(windowSize), Duration.ofMillis(windowSlide))).reduce(new ReduceFunction<Tuple2<Long, IntType>>() {
