 ...
 */
public class DataStreamWithSharedPartitionNodeITCase_RestartInjected_Randomized_456 {
    ...
    public void testJobWithSharePartitionNode() throws Exception {
        ...
        RestartFramework.at("during_custom_partitioning").on(flinkCluster).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DataStream<Integer> source = env.fromData(1, 2, 3, 4).partitionCustom(new TestPartitioner(), f -> f);
        source.addSink(new CollectSink("first"));
        source.addSink(new CollectSink("second")).setParallelism(2);
        JobClient jobClient = env.executeAsync("Shared Partition Node Test");
        Thread.sleep(300);
        jobClient.getJobExecutionResult().get();
        checkSinkResult("first-0", Arrays.asList(1, 2, 3, 4));
        checkSinkResult("second-0", Arrays.asList(1, 3));
        RestartFramework.at("after_job_submit").on(flinkCluster).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
