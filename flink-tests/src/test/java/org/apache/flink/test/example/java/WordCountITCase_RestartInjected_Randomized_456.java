 ...
 * Test {@link WordCount}.
 */
public class WordCountITCase_RestartInjected_Randomized_456 extends JavaProgramTestBaseJUnit4 {
    ...
    protected void testProgram() throws Exception {
        WordCount.main(new String[] { "--input", textPath, "--output", resultPath, "--execution-mode", "BATCH" });
    }
    ...
    public void testJobWithoutObjectReuse() throws Exception {
        ...
        RestartFramework.at("after_job_submit_no_reuse").on(MINI_CLUSTER_RESOURCE).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        testThread.start();
        Thread.sleep(500);
        testThread.join();
        org.junit.Assert.assertNotNull("The test program never triggered an execution.", this.latestExecutionResult);
        RestartFramework.at("during_job_running_no_reuse").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
            postSubmit();
        } catch (Exception e) {
            System.err.println(e.getMessage());
