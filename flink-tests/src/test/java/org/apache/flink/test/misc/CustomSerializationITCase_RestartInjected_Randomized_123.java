...
public class CustomSerializationITCase_RestartInjected_Randomized_123 extends TestLogger {

    private static final int PARLLELISM = 5;

    @ClassRule
    public static final MiniClusterWithClientResource MINI_CLUSTER_RESOURCE = new MiniClusterWithClientResource(new MiniClusterResourceConfiguration.Builder().setConfiguration(getConfiguration()).setNumberTaskManagers(1).setNumberSlotsPerTaskManager(PARLLELISM).build());
    ...
    public void testIncorrectSerializer1() throws Exception {
        try {
            ...
            env.fromSequence(1, 10 * PARLLELISM).map(new MapFunction<Long, ConsumesTooMuch>() {

                @Override
                public ConsumesTooMuch map(Long value) throws Exception {
                    return new ConsumesTooMuch();
                }
            }).rebalance().sinkTo(new DiscardingSink<>());
            ...
            assertTrue(findThrowable(e, candidate -> candidate.getMessage().contains("broken serialization.")).isPresent());
        }
    }
    ...
    public void testIncorrectSerializer2() throws Exception {
        try {
            StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
            env.setParallelism(PARLLELISM);
            env.fromSequence(1, 10 * PARLLELISM).map(new MapFunction<Long, ConsumesTooMuchSpanning>() {
    ...
    public void testIncorrectSerializer3() throws Exception {
        try {
            StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
            env.setParallelism(PARLLELISM);
            env.fromSequence(1, 10 * PARLLELISM).map(new MapFunction<Long, ConsumesTooLittle>() {
    ...
    public void testIncorrectSerializer4() throws Exception {
        try {
            StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
            env.setParallelism(PARLLELISM);
            env.fromSequence(1, 10 * PARLLELISM).map(new MapFunction<Long, ConsumesTooLittleSpanning>() {
