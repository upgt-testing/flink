 ...
 */
public class DataStreamBatchExecutionITCase_RestartInjected_Randomized_123 {

    private static final int DEFAULT_PARALLELISM = 1;

    @ClassRule
    public static MiniClusterWithClientResource miniClusterResource = new MiniClusterWithClientResource(new MiniClusterResourceConfiguration.Builder().setNumberTaskManagers(1).setNumberSlotsPerTaskManager(DEFAULT_PARALLELISM).build());
    ...
    public void batchFailoverWithKeyByBarrier() throws Exception {
        ...
        RestartFramework.at("before_batch_execute").on(miniClusterResource).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        SingleOutputStreamOperator<String> mapped = source.map(new SuffixAttemptId("a")).map(new SuffixAttemptId("b")).keyBy(in -> in).map(new SuffixAttemptId("c")).map(new OnceFailingMapper("d"));
        try (CloseableIterator<String> result = mapped.executeAndCollect()) {
            RestartFramework.at("during_batch_execution").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // only the operators after the key-by "barrier" are restarted and will have the
            // "attempt 1" suffix
            assertThat(iteratorToList(result), containsInAnyOrder("foo-a0-b0-c1-d1", "bar-a0-b0-c1-d1"));
        }
    }
    ...
    public void batchFailoverWithRebalanceBarrier() throws Exception {
        ...
        final StreamExecutionEnvironment env = getExecutionEnvironment();
        DataStreamSource<String> source = env.fromData("foo", "bar");
        SingleOutputStreamOperator<String> mapped = source.map(new SuffixAttemptId("a")).map(new SuffixAttemptId("b")).rebalance().map(new SuffixAttemptId("c")).map(new OnceFailingMapper("d"));
        try (CloseableIterator<String> result = mapped.executeAndCollect()) {
            RestartFramework.at("during_batch_execution").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void batchFailoverWithRescaleBarrier() throws Exception {
        ...
        SingleOutputStreamOperator<String> mapped = source.map(new SuffixAttemptId("a")).map(new SuffixAttemptId("b")).rescale().map(new SuffixAttemptId("c")).setParallelism(2).map(new OnceFailingMapper("d")).setParallelism(2);
        try (CloseableIterator<String> result = mapped.executeAndCollect()) {
            RestartFramework.at("during_batch_execution").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void batchReduceSingleResultPerKey() throws Exception {
        ...
        try (CloseableIterator<Long> sumsIterator = sums.executeAndCollect()) {
            RestartFramework.at("during_reduce_execution").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void batchSumSingleResultPerKey() throws Exception {
        ...
        try (CloseableIterator<Long> sumsIterator = sums.executeAndCollect()) {
            RestartFramework.at("during_sum_execution").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            List<Long> results = CollectionUtil.iteratorToList(sumsIterator);
            assertThat(results, equalTo(Arrays.asList(30L, 25L)));
        }
    }
    ...
    public void batchKeyedNonKeyedTwoInputOperator() throws Exception {
        ...
        DataStream<Tuple2<String, Integer>> keyedInput = env.fromData(Tuple2.of("regular2", 4), Tuple2.of("regular1", 3), Tuple2.of("regular1", 2), Tuple2.of("regular2", 1)).assignTimestampsAndWatermarks(WatermarkStrategy.<Tuple2<String, Integer>>forMonotonousTimestamps().withTimestampAssigner((in, ts) -> in.f1));
        RestartFramework.at("before_two_input_execute").on(miniClusterResource).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DataStream<Tuple2<String, Integer>> regularInput = env.fromData(Tuple2.of("regular4", 4), Tuple2.of("regular3", 3), Tuple2.of("regular3", 2), Tuple2.of("regular4", 1)).assignTimestampsAndWatermarks(WatermarkStrategy.<Tuple2<String, Integer>>forMonotonousTimestamps().withTimestampAssigner((in, ts) -> in.f1));
        ...
        try (CloseableIterator<String> resultIterator = result.executeAndCollect()) {
            RestartFramework.at("during_two_input_execution").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void batchKeyedBroadcastExecution() throws Exception {
        ...
        env.setParallelism(1);
        env.setRuntimeMode(RuntimeExecutionMode.BATCH);
        DataStream<Tuple2<String, Integer>> bcInput = env.fromData(Tuple2.of("bc1", 1), Tuple2.of("bc2", 2), Tuple2.of("bc3", 3)).assignTimestampsAndWatermarks(WatermarkStrategy.<Tuple2<String, Integer>>forMonotonousTimestamps().withTimestampAssigner((in, ts) -> in.f1));
        RestartFramework.at("before_broadcast_execute").on(miniClusterResource).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        DataStream<Tuple2<String, Integer>> regularInput = env.fromData(Tuple2.of("regular1", 1), Tuple2.of("regular1", 2), Tuple2.of("regular2", 2), Tuple2.of("regular1", 3), Tuple2.of("regular1", 4), Tuple2.of("regular1", 3), Tuple2.of("regular2", 5), Tuple2.of("regular1", 5), Tuple2.of("regular2", 3), Tuple2.of("regular1", 3)).assignTimestampsAndWatermarks(WatermarkStrategy.<Tuple2<String, Integer>>forMonotonousTimestamps().withTimestampAssigner((in, ts) -> in.f1));
        ...
        try (CloseableIterator<String> resultIterator = result.executeAndCollect()) {
            RestartFramework.at("during_broadcast_execution").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            List<String> results = CollectionUtil.iteratorToList(resultIterator);
    ...
    public void batchBroadcastExecution() throws Exception {
        ...
        env.setRuntimeMode(RuntimeExecutionMode.BATCH);
        DataStream<Tuple2<String, Integer>> bcInput = env.fromData(Tuple2.of("bc1", 1), Tuple2.of("bc2", 2), Tuple2.of("bc3", 3)).assignTimestampsAndWatermarks(WatermarkStrategy.<Tuple2<String, Integer>>forMonotonousTimestamps().withTimestampAssigner((in, ts) -> in.f1));
        DataStream<Tuple2<String, Integer>> regularInput = env.fromData(Tuple2.of("regular1", 1), Tuple2.of("regular1", 2), Tuple2.of("regular1", 3), Tuple2.of("regular1", 4), Tuple2.of("regular1", 3), Tuple2.of("regular1", 5), Tuple2.of("regular1", 3)).assignTimestampsAndWatermarks(WatermarkStrategy.<Tuple2<String, Integer>>forMonotonousTimestamps().withTimestampAssigner((in, ts) -> in.f1));
        BroadcastStream<Tuple2<String, Integer>> broadcastStream = bcInput.broadcast(STATE_DESCRIPTOR);
    ...
    public void batchMixedKeyedAndNonKeyedTwoInputOperator() throws Exception {
        ...
        env.setParallelism(1);
        env.setRuntimeMode(RuntimeExecutionMode.BATCH);
        DataStream<Tuple2<String, Integer>> bcInput = env.fromData(Tuple2.of("bc3", 3), Tuple2.of("bc2", 2), Tuple2.of("bc1", 1)).assignTimestampsAndWatermarks(WatermarkStrategy.<Tuple2<String, Integer>>forMonotonousTimestamps().withTimestampAssigner((in, ts) -> in.f1)).broadcast();
        DataStream<Tuple2<String, Integer>> regularInput = env.fromData(Tuple2.of("regular1", 1), Tuple2.of("regular1", 2), Tuple2.of("regular1", 3), Tuple2.of("regular1", 4), Tuple2.of("regular2", 3), Tuple2.of("regular2", 5), Tuple2.of("regular1", 3)).assignTimestampsAndWatermarks(WatermarkStrategy.<Tuple2<String, Integer>>forMonotonousTimestamps().withTimestampAssigner((in, ts) -> in.f1)).keyBy(input -> input.f0);
        ...
        RestartFramework.at("before_mixed_two_input_execute").on(miniClusterResource).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        try (CloseableIterator<String> resultIterator = result.executeAndCollect()) {
            RestartFramework.at("during_mixed_two_input_execution").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            List<String> results = CollectionUtil.iteratorToList(resultIterator);
    ...
    public void batchMixedKeyedAndNonKeyedMultiInputOperator() throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.setRuntimeMode(RuntimeExecutionMode.BATCH);
        DataStream<Tuple2<String, Integer>> bc1Input = env.fromData(Tuple2.of("bc3", 3), Tuple2.of("bc2", 2)).assignTimestampsAndWatermarks(WatermarkStrategy.<Tuple2<String, Integer>>forMonotonousTimestamps().withTimestampAssigner((in, ts) -> in.f1)).broadcast();
        DataStream<Tuple2<String, Integer>> bc2Input = env.fromData(Tuple2.of("bc1", 1)).assignTimestampsAndWatermarks(WatermarkStrategy.<Tuple2<String, Integer>>forMonotonousTimestamps().withTimestampAssigner((in, ts) -> in.f1)).broadcast();
        DataStream<Tuple2<String, Integer>> regularInput = env.fromData(Tuple2.of("regular1", 1), Tuple2.of("regular1", 2), Tuple2.of("regular1", 3), Tuple2.of("regular1", 4), Tuple2.of("regular2", 3), Tuple2.of("regular2", 5), Tuple2.of("regular1", 3)).assignTimestampsAndWatermarks(WatermarkStrategy.<Tuple2<String, Integer>>forMonotonousTimestamps().withTimestampAssigner((in, ts) -> in.f1)).keyBy(input -> input.f0);
        ...
        RestartFramework.at("before_multi_input_execute").on(miniClusterResource).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try (CloseableIterator<String> resultIterator = result.executeAndCollect()) {
            RestartFramework.at("during_multi_input_execution").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            List<String> results = CollectionUtil.iteratorToList(resultIterator);
            assertThat(results, equalTo(Arrays.asList("(regular1,1): [bc3, bc2, bc1]", "(regular1,2): [bc3, bc2, bc1]", "(regular1,3): [bc3, bc2, bc1]", "(regular1,3): [bc3, bc2, bc1]", "(regular1,4): [bc3, bc2, bc1]", "(regular2,3): [bc3, bc2, bc1]", "(regular2,5): [bc3, bc2, bc1]")));
        }
    }
    ...
    private static final class TestMixedTwoInputOperator extends AbstractStreamOperator<String> implements TwoInputStreamOperator<Tuple2<String, Integer>, Tuple2<String, Integer>, String> {
        ...
        @Override
        public void processElement1(StreamRecord<Tuple2<String, Integer>> element) throws Exception {
            ListState<String> operatorState = getOperatorStateBackend().getListState(LIST_STATE_DESCRIPTOR);
            output.collect(new StreamRecord<>(element.getValue() + ": " + operatorState.get().toString()));
        }
    ...
    private static class TestMixedMultipleInputOperator extends AbstractStreamOperatorV2<String> implements MultipleInputStreamOperator<String> {
        ...
        public List<Input> getInputs() {
            return Arrays.asList(new AbstractInput<Tuple2<String, Integer>, String>(this, 1) {
