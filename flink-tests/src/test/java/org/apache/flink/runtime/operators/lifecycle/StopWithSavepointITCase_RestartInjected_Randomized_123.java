...
public class StopWithSavepointITCase_RestartInjected_Randomized_123 extends AbstractTestBaseJUnit4 {
    ...
    public void test() throws Exception {
        ...
        }, env -> CheckpointStorageUtils.configureFileSystemCheckpointStorage(env, TEMPORARY_FOLDER.newFolder().toURI()));
        RestartFramework.at("before_stop_with_savepoint_execute").on(MINI_CLUSTER_RESOURCE).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        TestJobExecutor executor = TestJobExecutor.execute(testJob, MINI_CLUSTER_RESOURCE).waitForEvent(WatermarkReceivedEvent.class);
        executor.stopWithSavepoint(temporaryFolder, withDrain);
        RestartFramework.at("during_stop_with_savepoint_execute").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        SameCheckpointValidator sameCheckpointValidator = // note: using highest checkpoint will not work with partially finishing tasks
        ...
        if (withDrain) {
            checkOperatorsLifecycle(testJob, sameCheckpointValidator, new DrainingValidator(), /* Currently (1.14), finish is only called with drain; */
    ...
    public static Object[] parameters() {
        return new Object[][] { new Object[] { true, SIMPLE_GRAPH_BUILDER }, new Object[] { false, SIMPLE_GRAPH_BUILDER }, new Object[] { true, COMPLEX_GRAPH_BUILDER }, new Object[] { false, COMPLEX_GRAPH_BUILDER } };
    }
