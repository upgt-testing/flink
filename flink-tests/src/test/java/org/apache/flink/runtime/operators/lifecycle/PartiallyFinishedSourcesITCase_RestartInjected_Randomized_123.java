...
public class PartiallyFinishedSourcesITCase_RestartInjected_Randomized_123 extends TestLogger {
    ...
    public void init() throws Exception {
        ...
        FsStateChangelogStorageFactory.configure(configuration, TEMPORARY_FOLDER.newFolder(), Duration.ofMinutes(1), 10);
    ...
    public void test() throws Exception {
        ...
        RestartFramework.at("before_partially_finished_execute").on(miniClusterResource).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("during_partially_finished_execute").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        String finishingOperatorID = iterator.next();
        JobVertexID finishingVertexID = findJobVertexID(testJob, finishingOperatorID);
        TestJobExecutor executor = TestJobExecutor.execute(testJob, miniClusterResource).waitForEvent(CheckpointCompletedEvent.class);
        executor = executor.sendOperatorCommand(finishingOperatorID, FINISH_SOURCES, subtaskScope).waitForSubtasksToFinish(finishingVertexID, subtaskScope).// wait for a checkpoint to complete with a finished subtask(s)
        // but skip one checkpoint that might be started before finishing
        waitForEvent(CheckpointCompletedEvent.class).waitForEvent(CheckpointCompletedEvent.class);
        if (failover) {
            ...
            executor.triggerFailover(subtaskScope == ALL_SUBTASKS ? iterator.next() : finishingOperatorID);
        }
    ...
    private TestJobWithDescription buildJob() throws Exception {
        return graphBuilder.build(sharedObjects, cfg -> {
        }, env -> {
            ...
            CheckpointStorageUtils.configureFileSystemCheckpointStorage(env, TEMPORARY_FOLDER.newFolder().toURI());
        });
    }

    private JobVertexID findJobVertexID(TestJobWithDescription testJob, String finishingOperatorID) {
        return stream(testJob.jobGraph.getVertices().spliterator(), false).filter(v -> v.getOperatorIDs().stream().anyMatch(idPair -> matches(idPair, finishingOperatorID))).findAny().orElseThrow(() -> new RuntimeException("Vertex not found: " + finishingOperatorID)).getID();
    }
