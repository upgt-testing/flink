...
public class BoundedSourceITCase_RestartInjected_Randomized_123 extends TestLogger {
    ...
    private static Configuration configuration() {
        ...
        try {
            FsStateChangelogStorageFactory.configure(conf, TEMPORARY_FOLDER.newFolder(), Duration.ofMinutes(1), 10);
        } catch (IOException e) {
    ...
    public void test() throws Exception {
        RestartFramework.at("during_bounded_source_execute").on(miniClusterResource).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("before_bounded_source_execute").on(miniClusterResource).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
