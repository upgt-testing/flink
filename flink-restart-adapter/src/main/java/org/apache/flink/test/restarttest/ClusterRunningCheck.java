...
/**
 * Health check to verify the Flink MiniCluster is running.
 */
public class ClusterRunningCheck implements HealthCheck<MiniClusterWithClientResource> {
    ...
    public HealthCheckResult checkHealth(MiniClusterWithClientResource cluster) throws Exception {
        ...
        boolean isRunning = cluster.getMiniCluster().isRunning();
        result.addMetric("is_running", isRunning);

        if (!isRunning) {
            result.addFailure("MiniCluster is not running");
        }
