 ...
 * HDFS.
 */
public class DistributedCacheDfsTest_RestartInjected_Randomized_456 extends TestLogger {
    ...
    public void testDistributedFileViaDFS() throws Exception {
        RestartFramework.at("before_dfs_execute").on(MINI_CLUSTER_RESOURCE).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        StreamExecutionEnvironment env = createJobWithRegisteredCachedFiles();
        org.apache.flink.core.execution.JobClient jobClient = env.executeAsync("Distributed Cache Via Blob Test Program");
        RestartFramework.at("during_dfs_execute").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testSubmittingJobViaRestClusterClient() throws Exception {
        ...
        RestartFramework.at("during_rest_client_execute").on(MINI_CLUSTER_RESOURCE).restart("taskmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        java.util.concurrent.CompletableFuture<JobResult> jobResultFuture = restClusterClient.submitJob(jobGraph).thenCompose(restClusterClient::requestJobResult);
        RestartFramework.at("before_rest_client_submit").on(MINI_CLUSTER_RESOURCE).restart("jobmanager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
